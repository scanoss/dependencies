...
func TestAllUrlsSearch(t *testing.T) {
	...
	q := database.NewDBSelectContext(s, db, conn, myConfig.Database.Trace)
	allUrlsModel := NewAllURLModel(ctx, s, conn, models.NewProjectModel(ctx, s, conn, q),
		NewGolangProjectModel(ctx, s, db, conn, myConfig), q)
