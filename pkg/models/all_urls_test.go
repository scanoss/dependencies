...
func TestAllUrlsSearchNoLicense(t *testing.T) {
	...
	allUrls, err := allUrlsModel.GetUrlsByPurlString("pkg:gem/taballa.hp-PD/tablestyle@0.0.1")
	if err != nil {
		t.Errorf("all_urls.GetUrlsByPurlString() error = %v", err)
	}
	if len(allUrls) < 1 {
...
func TestAllUrlsSearchBadSql(t *testing.T) {
	...
	_, err = allUrlsModel.GetUrlsByPurlString("pkg:gem/taballa.hp-PD/tablestyle@0.0.7")
	if err == nil {
		t.Errorf("all_urls.GetUrlsByPurlString() error = did not get an error")
	} else {
		fmt.Printf("Got expected error = %v\n", err)
